...
DESIGN INTENT:
  The retrieval tools return raw context chunks — they do NOT call an
  LLM internally. The AI (Claude Code or LM Studio) that called the
  tool does the reasoning and answer generation itself, grounded in
  the returned context. This is the correct MCP pattern.
...
# Ensure the graph_rag directory is on the path so all modules resolve
...
def retrieve_context(
    ...
    in the knowledge base. The returned context should be used to ground
    your answer.
    ...
    Returns:
        Formatted text chunks with similarity scores, ready to use as context.
        Returns a clear message if no relevant content is found.
    """
    try:
...
def ingest_document(file_path: str) -> str:
    ...
    The document will be split into chunks, embedded, stored in the
    database, and its entities added to the knowledge graph — all
    automatically.
    ...
    Returns:
        Confirmation with the number of chunks created, or an error message.
    """
    try:
...
def get_knowledge_base_stats() -> str:
    ...
    Shows how many documents, chunks, embeddings, graph nodes,
    and graph edges are currently stored.

    Returns:
        A formatted summary of the knowledge base contents.
    """
    try:
...
def build_knowledge_graph() -> str:
    ...
    seems incomplete. It re-extracts entities and relationships from
    every stored chunk and upserts them into the graph tables.
...
def search_chunks(query: str, k: int = 8, min_similarity: float = 0.3) -> str:
    ...
    expansion or LLM processing. Useful for exploring what the knowledge
    base contains on a topic.
...
def main():
    parser = argparse.ArgumentParser(
        description="Graph RAG MCP Server",
    ...
    parser.add_argument(
        "--http",
        action="store_true",
        help="Run as HTTP server (for LM Studio). Default: stdio (for Claude Code).",
    )
    parser.add_argument(
        "--port",
        type=int,
