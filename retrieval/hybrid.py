...
def retrieve(
    ...
    Returns list of dicts sorted by similarity (seeds first, graph-expanded after).
    Each dict: {chunk_id, document_id, content, similarity, source}
    """
    ...
    # Step 3: fetch expanded chunks from DB
    expanded_chunks = []
    if new_ids:
        with get_conn() as conn:
            expanded_chunks = queries.get_chunks_by_ids(conn, new_ids)

    # Combine: vector results first (with similarity score), then graph-expanded
    combined = list(vector_results)  # already has chunk_id, document_id, content, similarity
    ...
    for chunk in expanded_chunks:
        combined.append({
            ...
            "similarity": 0.0,  # graph-expanded, no direct similarity score
            "source": "graph",
        })
